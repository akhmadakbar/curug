...
class Xcrud
{
    ...
    public static function get_requested_instance()
    {
        ...
        if (isset($_SESSION['xcrud_session'][$inst_name]['key']) && $_SESSION['xcrud_session'][$inst_name]['key'] == $key)
        {
			...
			// echo("<pre>"); // akbar
			// print_r(self::$instance[$inst_name]);
			// echo("</pre>");
			// die();
    ...
    public function relation($fields = '', $rel_tbl = '', $rel_field = '', $rel_name = '', $rel_where = array(), $order_by = false,
        $multi = false, $rel_concat_separator = ' ', $tree = false, $depend_field = '', $depend_on = '', $not_in = '')
    {
            ...
            case 'print':
                if (!$this->is_print)
                {
                    return self::error('Restricted');
								...
								// Tambahan Agar Cetak Report menjadi Export Excel (abr)
        ...
        else
        {
			if (trim(strtolower(strrchr($path, '.')), '.') == 'pdf')
            {
				...
				if (isset($settings['path']))
				{
					...
					// die($this->resolvePath($alamat.$this->upload_folder_def.'/'.$image));				
					header("Location: ".$this->resolvePath($alamat.$settings['path'].'/'.$image)); /* Redirect browser */
				...
				else
				{
					...
					$alamat = $alamat.str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
					// die($this->resolvePath($alamat.$this->upload_folder_def.'/'.$image));				
					header("Location: ".$this->resolvePath($alamat.$this->upload_folder_def.'/'.$image)); /* Redirect browser */
					// $path = $this->check_folder(Xcrud_config::$upload_folder_def, 'get_image_folder');
				}
	...
	public function resolvePath($path,  $separator = '\\/') {
